...
/mob/living/Hear(atom/movable/speaker, datum/language/message_language, raw_message, radio_freq, radio_freq_name, radio_freq_color, list/spans, list/message_mods = list(), message_range=0)
	...
	if(message_range != INFINITY && dist > eavesdrop_range && !HAS_TRAIT(src, TRAIT_GOOD_HEARING))
		...
		if(speaker_is_signing)
			deaf_message = "[span_name("[chat_name_color_prefs_check(speaker, src, speaker_name)]")] [speaker.get_default_say_verb()] something, but the motions are too subtle to make out from afar." // OUTERBOUNDS EDIT - Colored chat names - ORIGINAL: deaf_message = "[span_name("[speaker]")] [speaker.get_default_say_verb()] something, but the motions are too subtle to make out from afar."
			...
			deaf_message = "[span_name("[chat_name_color_prefs_check(speaker, src, speaker_name)]")] [speaker.verb_whisper] something, but you are too far away to hear [speaker.p_them()]." // OUTERBOUNDS EDIT - Colored chat names - ORIGNAL: deaf_message = "[span_name("[speaker]")] [speaker.verb_whisper] something, but you are too far away to hear [speaker.p_them()]."
	...
	if(speaker != src)
		if(!radio_freq) //These checks have to be separate, else people talking on the radio will make "You can't hear yourself!" appear when hearing people over the radio while deaf.
			deaf_message = "[span_name("[chat_name_color_prefs_check(speaker, src)]")] [speaker.get_default_say_verb()] something but you cannot hear [speaker.p_them()]." // OUTERBOUNDS EDIT - Colored chat names - ORIGINAL: deaf_message = "[span_name("[speaker]")] [speaker.get_default_say_verb()] something but you cannot hear [speaker.p_them()]."
			deaf_type = MSG_VISUAL
	else
